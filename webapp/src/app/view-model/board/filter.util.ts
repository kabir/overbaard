...
export class AllFilters {
  ...
  constructor(boardFilters: BoardFilterState, searchFilters: BoardSearchFilterState,
              projectState: ProjectState, customFieldMetadata: Map<string, CustomFieldMetadata>,
              currentUser: string) {
    ...
    this._customFieldFilters = Map<string, SimpleFilter>().withMutations(mutable => {
      boardFilters.customField.forEach((f, k) => {
        // If it is a type=user custom field, we need to include the current user search
        const cffa: FilterAttributes = FilterAttributesUtil.createCustomFieldFilterAttributes(k, customFieldMetadata.get(k));

        const cfFilter: SimpleFilter = cffa.hasCurrentUser ?
          new CurrentUserFilter(cffa, f, currentUser) : new SimpleFilter(cffa, f);
